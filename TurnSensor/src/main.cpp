...
void loop()
{
  if (buttonB.getSingleDebouncedRelease()) // Linksrum Drehen
  {
    display.clear();
    ...
    display.print(F("   "));
      ...
      else if( turnAngle == turnAngle90  || bumpSensors.rightIsPressed() || bumpSensors.leftIsPressed())
      {
  ...
  else if (buttonC.getSingleDebouncedRelease()) // Fahre so lange, auser bumpSensor aktiv, dann stehen bleiben
  {
    motors.setSpeeds(0, 0);
    display.clear();
    display.gotoXY(0, 0);
